...
const Page = module.exports = class Page
{
    ...
    async load( scope = 'list' )
    {
        ...
        if( this.#data.title?.[locale] === undefined )
        {
            ...
            {
                page.tags = await Promise.all( Object.keys( page.tags ).map( id => Tag.get( this.#ctx, parseInt( id ), page.tags[id] )));
                //page.author = await Promise.all( Object.keys( page.tags ).map( id => Tag.get( this.#ctx, parseInt( id ), page.tags[id] ))); TODO
